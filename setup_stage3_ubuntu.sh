#!/bin/bash
#
# setup_stage3_ubuntu.sh builds an initram image based on the Ubuntu release
# named in $UBUNTU_RELEASE (see config.sh) with all packages upgraded to their
# latest versions, that includes M-Lab configs and scripts, epoxy_client and
# k8s-related binaries
#
# Example:
...
if ! test -f $BOOTSTRAP/build.date ; then
    ...
    # NOTE: systemd-resolved is a separate package in releases after 22.04, and
    # is required for name resolution since the systemd-networkd configs
    # generated by generate_network_config.sh define the DNS servers.
